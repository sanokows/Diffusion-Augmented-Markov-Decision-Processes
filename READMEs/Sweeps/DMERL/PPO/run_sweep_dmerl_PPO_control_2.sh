...
for ENV_NAME in "${ENV_NAMES[@]}"; do
    ...
    CUDA_VISIBLE_DEVICES=$GPU_ID python -m src.jaxrl.reppo_DiffPPO \
        env.name="$ENV_NAME" \
        wandb.project_suffix="_FR_test_PPO" \
        ...
        hyperparameters.temperature_lr=1e-4 \
        hyperparameters.entropy_coef=0.0004 \
        ...
        hyperparameters.use_categorical_value=true \
        hyperparameters.normalize_advantages=false \
        hyperparameters.ent_target_mult=8. \
        hyperparameters.num_collection_step_factor=0.5 \
        ...
        hyperparameters.diffusion.init_std=3. \
        env=mjx_dmc \
        trials=5 \
        seed=0 &
    GPU_PIDS[$GPU_ID]=$!
