...
for ENV_NAME in "${ENV_NAMES[@]}"; do
    ...
    CUDA_VISIBLE_DEVICES=$GPU_ID python -m src.jaxrl.reppo_DMERL_new \
        env.name="$ENV_NAME" \
        wandb.project_suffix="_FR_21_01" \
        ...
        seed=0 \
        num_trials=4 \
        experiment_overrides=mjx_dmc_large_data_dmerl_WPO_hard_envs &
    GPU_PIDS[$GPU_ID]=$!
