...
python src/jaxrl/reppo_dime.py -cn ${CONFIG} env=mjx_humanoid_dime env.name=${ENV_NAME} seed=${USING_SEED} experiment_overrides=${MJX_DMC_DATA} wandb.entity=${ENTITY} env.vmin=${VMIN} env.vmax=${VMAX}
EOF"
                done
            done
        done
