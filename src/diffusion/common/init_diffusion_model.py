...
from src.diffusion.common.learning_rate_scheduler import get_learning_rate_scheduler, get_learning_rate_scheduler_with_end
from src.diffusion.common.models.pisgrad_net import PISGRADNet
from src.diffusion.common.utils import flattened_traversal
    ...
    if alg_cfg.learn_prior:
        def prior_log_prob(x, params):
            log_probs = distrax.MultivariateNormalDiag(params['params']['prior_mean'],
                                                       jnp.ones(dim) * jax.nn.softplus(params['params']['prior_std'])).log_prob(x)
            return log_probs
    else:
        def prior_log_prob(x, params):
...
def init_model(key, params, cfg, dim, obs_dim, learn_forward=True, learn_backward=True):
    ...
    if cfg.use_step_size_scheduler:
        # Check if separate learning rates are defined
        if hasattr(cfg.alg.optimizer, 'lr_actor_end') and hasattr(cfg.alg.optimizer, 'lr_critic_end'):
            # Use separate learning rates with custom end values
            actor_lr_init = cfg.alg.optimizer.lr_actor
            actor_lr_end = cfg.alg.optimizer.lr_actor_end
            critic_lr_init = cfg.alg.optimizer.lr_critic  
            critic_lr_end = cfg.alg.optimizer.lr_critic_end
            
            model_opt = optax.masked(optax.adam(get_learning_rate_scheduler_with_end(cfg, actor_lr_init, actor_lr_end),
                                                b1=cfg.optimizer.b1),
                                     mask=flattened_traversal(
                                         lambda path, _: ('fwd_params' in path) or ('bwd_params' in path)))
            betas_opt = optax.masked(optax.adam(get_learning_rate_scheduler_with_end(cfg, critic_lr_init, critic_lr_end),
                                                b1=cfg.optimizer.b1),
                                     mask=flattened_traversal(
        ...
        model_opt = optax.masked(optax.adam(cfg.optimizer.step_size, b1=cfg.optimizer.b1),
                                 mask=flattened_traversal(
                                     lambda path, _: ('fwd_params' in path) or ('bwd_params' in path)))
        betas_opt = optax.masked(optax.adam(cfg.optimizer.step_size_betas, b1=cfg.optimizer.b1),
                                 mask=flattened_traversal(
                                     lambda path, _: ('fwd_params' not in path) and ('bwd_params' not in path)))
