...
def init_hbs(key, cfg, dim, obs_dim, target=None):
    ...
    def forward_model(step, x, obs, vel, model_state, params, aux):
        langevin_vals = aux
        input = jnp.concatenate([x, vel], axis=0)
        return model_state.apply_fn[0](params['params']['fwd_params'], input, obs, step,
                                       jax.lax.stop_gradient(langevin_vals))

    def backward_model(step, x, obs, vel, model_state, params, aux):
        langevin_vals = aux
        input = jnp.concatenate([x, vel], axis=0)
        model_params = jax.lax.stop_gradient(params['params']['bwd_params']) if cfg.use_path_gradient else params['params']['bwd_params']
        return model_state.apply_fn[1](model_params, input, obs, step, jax.lax.stop_gradient(langevin_vals))
