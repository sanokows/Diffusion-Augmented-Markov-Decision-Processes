...
def sample_kernel(rng_key, mean, scale):
    ...
    # eps = jax.random.normal(rng_key, shape=(mean.shape[0], 1))
...
def plot_timesteps(diffusion_model, model_state, cfg):
    if cfg.use_wandb:
        ...
        dts = jax.vmap(dt_fn)(jnp.arange(cfg.diffusion_type.num_steps))
        fig, ax = plt.subplots()
        ax.plot(dts)
...
def get_sampler_init(alg_name):
    ...
    if alg_name == 'ddm':
        from src.dime_model.denoising_diffusion import init_denoising_diffusion
        return init_denoising_diffusion
